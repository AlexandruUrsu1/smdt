...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def tension_up(self):
        ...
        stride = 5
        result = stepper.step_to(300, 10, callback=self.update_ext_tension)
        result = stepper.step_to(target, 6, callback=self.update_ext_tension)
        stepper.pause()
        self.update_status("Measuring internal tension")
        tension, frequency =  self.tension_device.get_tension()
        self.update_int_tension(tension)
    ...
    def autotension(self):
        ...
        stepper.pause()
        self.update_status("Holding at 400")
        stepper.hold(400, 5, hold_time=10, callback=self.update_ext_tension)
        stepper.resume()
        self.update_status("Tensioning to 0")
        ...
        #Simple program to increase/decrease program after autotensioning (for full automated process). Still requires testing
        #Also need more specific requirements for when we would like the "AI" tensioning to kick in, very large range at the moment
        #if(result == 1):
        #    while(True):
        #        if(tension > 322.57): #need to double check value
        #            self.tension_down(self)
        #        elif(tension < 308.97): #need to double check value
        #            self.tension_up(self)
        #       else:
        #           break
